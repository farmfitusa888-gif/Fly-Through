...
**Status:** BUILT. Generated by `docs/shot_guide.py`; all acceptance criteria pass.
...
- [x] Regenerating after a change to `REQUIRED_ANCHORS` updates the guide with no manual edit
      — `test_guide_states_the_required_anchors_it_gets_from_code`
- [x] Every room name in the guide resolves through `rooms.resolve()` — 20/20, checked by the generator
- [x] Both audience versions produced from one source — `docs/shot_guide.py`
- [x] A shoot following the guide produces a plan with **zero** warnings
      — `test_a_guide_compliant_shoot_produces_zero_warnings`
- [x] Names the seam-matching rule explicitly in the photographer version
- [x] Names the aerial-side rule and the opposite-elevation rule, and breaking
      them trips the validator — `test_guide_rules_are_the_ones_the_validator_enforces`
