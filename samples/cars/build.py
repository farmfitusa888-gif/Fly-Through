...
def needed(write: bool = True) -> int:
    """Exactly which files are missing, by name, ready to hand to somebody.

    `status` answers "is this buildable"; this answers "which ones do I drop
    here". They are not the same question when the source is a 287 MB zip of a
    few hundred similarly-named files, and the difference is the whole reason
    nobody has dropped them yet.

    Every id, filename and duration below was checked against the live OpenArt
    account on 2026-08-25 -- all fifteen are present and completed there, and
    the manifest matches it exactly, so nothing here is a guess about what the
    zip contains.
    """
    lines = [
        "# Clips still needed",
    ...
    for slug, film in MANIFEST["films"].items():
        have = [b for b in film["beats"] if find_local(b)]
        miss = [b for b in film["beats"] if not find_local(b)]
        lines += [f"## {film['title']}  ({len(have)}/{len(film['beats'])} present)", ""]
        if not miss:
            lines += ["Complete. Nothing needed.", ""]
            continue
        lines += ["| beat | the cut | file to find |", "|---|---|---|"]
        for b in miss:
            lines.append(f"| {b['n']} | {b['from']} → {b['to']} | `{b['file']}` |")
        lines.append("")
        total += len(miss)
    lines += [f"**{total} files.** Once they land, `python3 samples/cars/build.py` "
              "cuts both films."]
